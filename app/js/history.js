...
const getUser = db.collection('requests').orderBy('responcedTime', 'desc').get().then((snapshot) => {

    snapshot.docs.forEach(doc => {
        let items = doc.data();

        db.collection('users').where('id', '==', items.receiveID).get().then((snap) => {
            snap.docs.forEach(element => {
                ...
                p1.textContent = "Response Time: ";
                p2.textContent = "Publish At: ";
                a1.textContent = items.responce;
                a12.textContent = responceTime.toLocaleDateString('vi-VN', options) + ' ----  ' + checkValid(responceTime.getHours()) + ":" + checkValid(responceTime.getMinutes()) + ":" + checkValid(responceTime.getSeconds());
                a13.textContent = publishAt.toLocaleDateString('vi-VN', options) + ' ---- ' + checkValid(publishAt.getHours()) + ":" + checkValid(publishAt.getMinutes()) + ":" + checkValid(publishAt.getSeconds());
